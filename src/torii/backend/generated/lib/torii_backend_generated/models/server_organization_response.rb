...
module ToriiBackendGenerated
  class ServerOrganizationResponse < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ToriiBackendGenerated::ServerOrganizationResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ToriiBackendGenerated::ServerOrganizationResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'role_set_id')
        self.role_set_id = attributes[:'role_set_id']
      end
    ...
    def hash
      [id, name, slug, role_set_id, public_metadata, private_metadata, created_at].hash
    end
